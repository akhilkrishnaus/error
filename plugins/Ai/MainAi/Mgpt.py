...
@Client.on_message(filters.command(["mgpt4", "mgpt"]))
async def lexica_askbot(client, message):
    query = message.text.split()[1:]
    query = " ".join(query)
    
    if not query:
        await message.reply_text("Usage: /mgpt4 who is your owner")
        return
    ...
    payload = {
        'messages': [
            {
                'role': "system",
                'content': "You are Malayalam talking assistant, you only talk malayalam you act like a malayalam talking assistant.Your name is Mr.tom . Your owner is Albert @aktelegram1.You are a stern person. Your developer is Albert.For Telegram, contact him at @aktelegram1. Owned by @aktelegram1.Albert GitHub: https://github.com/mallu-movie-world-dev1 check it out ✅",
            },
            {
                'role': "user",
                'content': query,
